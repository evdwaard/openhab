 ...
 */
package org.openhab.persistence.redis.internal;
...
public class RedisService implements PersistenceService {
	...
	public void store(Item item, String alias) {
		...
		try {
			...
			logger.debug("Storing to redis value [" + execCmd + "]");
			...
			reader.close();
			if (output.length() > 0) {
				logger.debug("Output from redis command is: " + output);
			}
			...
			reader.close();
			if (output.length() > 0) {
				logger.debug("Error from redis command is: " + output);
			}

			process.waitFor();

		} catch (Exception e) {
			logger.error("Could not redis command [" + execCmd + "]", e);
		} finally {
			try {
				reader.close();
				reader = null;
	...
	public void store(Item item) {
		throw new UnsupportedOperationException(
				"The Redis service requires aliases for persistence configurations that should match the Exec statement. Please configure exec.persist properly.");
	}
